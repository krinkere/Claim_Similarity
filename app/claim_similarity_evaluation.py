...
def process_huge_claims(claim_txt):
    ...
    blah = claim_txt.toPandas().values
    # status_paths_with_art_class = claim_txt.flatMap(lambda x: [x['text']]).collect()

    # claim_txt.collect().foreach(claim_corpus.append())
...
claim_txts = df_pati_clm_data
claim_txt_corpus, original_corpus = process_huge_claims(claim_txts)
